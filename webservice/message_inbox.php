...
$data = array('mail_data'=>$mail_inbox_data,'success'=>$success,'msg'=>$msg);
}
...
$data = array('mail_data'=>$mail_outbox_data,'success'=>$success,'msg'=>$msg);
}
elseif($view=='trash')
{
    $link=array();
   ...
   //          For Outbox          //
  ////////////////////////////////// 
    $qr1='SELECT (SELECT username FROM login_authentication WHERE USER_ID=ISTRASH AND PROFILE_ID=\''.UserWs('PROFILE_ID').'\') as ISTRASH,MAIL_ID FROM msg_outbox WHERE FROM_USER=\''.$userName.'\' ';
...
$data = array('mail_data'=>$mail_trash_info,'success'=>$success,'msg'=>$msg);
