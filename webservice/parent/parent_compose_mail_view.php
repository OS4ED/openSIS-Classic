...
$_SESSION['PROFILE_ID'] = $profile_id = $_REQUEST['profile_id'];
...
if(count($auth_data)>0)
{
    if($auth_data['user_id']==$parent_id && $auth_data['user_profile']=='parent')
    {
        ...
        $schools=$_SESSION['UserSchool'] = $_REQUEST['school_id']=$school_RET[1]['SCHOOL_ID'];
        //$allMP = $_REQUEST['mp_type'];
        //$teacher['UserMP'] = $mp_id = $_REQUEST['mp_id'];

        $user_id=$parent_id;
        $username_user=DBGet(DBQuery('SELECT USERNAME FROM login_authentication WHERE USER_ID='.$user_id.' AND PROFILE_ID='.$profile_id));
        ...
        $course_periods=DBGet(DBQuery('SELECT GROUP_CONCAT(course_period_id) as COURSE_PERIOD_ID FROM schedule WHERE STUDENT_ID='.$student_id));
        ...
        $tmp_q=DBGet(DBQuery('SELECT DISTINCT sjp.PERSON_ID FROM student_enrollment se,students_join_people sjp WHERE (se.START_DATE=\'0000-00-00\' OR se.START_DATE<=\''.date('Y-m-d').'\') AND (se.END_DATE=\'0000-00-00\' OR se.END_DATE IS NULL OR se.END_DATE>=\''.date('Y-m-d').'\') AND se.STUDENT_ID=sjp.STUDENT_ID AND sjp.STUDENT_ID='.$student_id));
        foreach($tmp_q as $tmp_a)
        {
           $tmp_p_arr[]=$tmp_a['PERSON_ID'];
        }
        $tmp_s=DBGet(DBQuery('SELECT DISTINCT sjp.STUDENT_ID FROM student_enrollment se,students_join_people sjp WHERE (se.START_DATE=\'0000-00-00\' OR se.START_DATE<=\''.date('Y-m-d').'\') AND (se.END_DATE=\'0000-00-00\' OR se.END_DATE IS NULL OR se.END_DATE>=\''.date('Y-m-d').'\') AND se.STUDENT_ID=sjp.STUDENT_ID AND sjp.PERSON_ID='.$parent_id));
        foreach($tmp_s as $tmp_st)
        {
           $tmp_stu_arr[]=$tmp_st['STUDENT_ID'];
        }
        if($course_periods!='')
        {
        ...
        $groupList = DBGet(DBQuery("SELECT GROUP_ID,GROUP_NAME FROM mail_group where user_name='".$username_user."'"));
        ...
        /*
        $RET = DBGet(DBQuery("SELECT cpv.ID,cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.TITLE,cp.SCHOOL_ID,cpv.PERIOD_ID FROM course_periods cp,course_period_var cpv WHERE cp.SYEAR='".UserSyear()."' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cp.SCHOOL_ID='".UserSchool()."' AND (MARKING_PERIOD_ID IN (".GetAllMPWs($allMP,$mp_id,UserSyear(),UserSchool()).") OR (MARKING_PERIOD_ID IS NULL AND BEGIN_DATE<='".date('Y-m-d')."' AND END_DATE>='".date('Y-m-d')."')) group by (cp.COURSE_PERIOD_ID)"));    // AND (TEACHER_ID='".$teacher_id."' OR SECONDARY_TEACHER_ID='".$teacher_id."')
