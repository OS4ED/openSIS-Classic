...
if(count($auth_data)>0)
{
    if($auth_data['user_id']==$parent_id && $auth_data['user_profile']=='parent')
    {
            ...
            if(count($assignments_RET)>0)
            {
                ...
                foreach($assignments_RET as $school_date=>$events)
                {
                    ...
                    foreach($events as $event)
                    {
                        ...
                        if(isset($event['DESCRIPTION']) && $event['DESCRIPTION']!='NULL')
                        {
                            $event['DESCRIPTION']=html_entity_decode(html_entity_decode($event['DESCRIPTION']));
        ...
        $data = array('selected_student'=>$student_id,'school_calender'=>$calender,'cal_success'=>$cal_success,'cal_msg'=>$cal_msg,'cal_data'=>$cal_data,'data_success'=>$data_success,'data_success_msg'=>$data_success_msg,'selected_calender_id'=>$_REQUEST['calendar_id']);
    }
    else 
    {
