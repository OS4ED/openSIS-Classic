...
if(count($auth_data)>0)
{
    ...
    {
        $student_id=$_SESSION['student_id'] = $_SESSION['STUDENT_ID'] = $_REQUEST['student_id'];
        ...
        //        }    
                ...
                $tmp_q=DBGet(DBQuery('SELECT DISTINCT la.USERNAME FROM login_authentication la,student_enrollment se,students_join_people sjp WHERE (se.START_DATE=\'0000-00-00\' OR se.START_DATE<=\''.date('Y-m-d').'\') AND (se.END_DATE=\'0000-00-00\' OR se.END_DATE IS NULL OR se.END_DATE>=\''.date('Y-m-d').'\') AND se.STUDENT_ID=sjp.STUDENT_ID AND sjp.STUDENT_ID='.$student_id.' AND sjp.PERSON_ID=la.USER_ID AND la.USERNAME IS NOT NULL AND la.PROFILE_ID=4 '));
                foreach($tmp_q as $tmp_a)
                $tmp_arr[]=$tmp_a['USERNAME'];
        ...
        //
        //        }
                $tmp_own=DBGet(DBQuery('SELECT DISTINCT la.USERNAME FROM login_authentication la WHERE la.PROFILE_ID = '.$profile_id.' AND la.USER_ID = '.$user_id.' AND la.USERNAME IS NOT NULL'));
        ...
        $data = array('selected_student'=>$student_id,'msg'=>$msg);
    }
    else 
    {
