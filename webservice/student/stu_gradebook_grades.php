...
if(count($auth_data)>0)
{
    if($auth_data['user_id']==$student_id && $auth_data['user_profile']=='student')
    {
        ...
        if($action_type == 'totals')
        {
            ...
            $courses_RET = DBGet(DBQuery('SELECT c.TITLE AS COURSE_TITLE,cp.TITLE,cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.TEACHER_ID AS STAFF_ID FROM schedule s,course_periods cp,courses c WHERE s.course_period_id = cp.course_period_id  AND c.COURSE_ID=cp.COURSE_ID AND s.SYEAR=\''.UserSyear().'\' AND cp.COURSE_PERIOD_ID=s.COURSE_PERIOD_ID AND ((cp.MARKING_PERIOD_ID IN ('.GetAllMPWs($MP_TYPE,UserMP(),UserSyear(),UserSchool()).') OR cp.MARKING_PERIOD_ID IS NULL) AND 
                                            ((\''.date('Y-m-d',strtotime(DBDate())).'\' BETWEEN s.START_DATE AND s.END_DATE OR \''.date('Y-m-d',strtotime(DBDate())).'\'>=s.START_DATE AND s.END_DATE IS NULL))) AND
                     s.STUDENT_ID=\''.$student_id.'\' ORDER BY cp.COURSE_ID'),array(),array('COURSE_PERIOD_ID')); // AND ((\''.date('Y-m-d',strtotime(DBDate())).'\' BETWEEN s.START_DATE AND s.END_DATE OR \''.date('Y-m-d',strtotime(DBDate())).'\'>=s.START_DATE AND s.END_DATE IS NULL)))
