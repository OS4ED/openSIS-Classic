...
if(count($auth_data)>0)
{
    if($auth_data['user_id']==$student_id && $auth_data['user_profile']=='student')
    {
        ...
        foreach($periods_RET as $period)
        {
            ...
            $period_data[$i]['START_TIME'] = $period['START_TIME'];
            $period_data[$i]['END_TIME'] = $period['END_TIME'];
            $i++;
        }
        ...
        foreach($absences_RET as $school_date=>$absences)
        {
            ...
            $days_RET[$i]['DAILY'] = _makeStateValue($absences[key($absences)][1]['STATE_VALUE']);
             $days_RET[$i]['DAILY_STATE_CODE'] = _makeStateCode($absences[key($absences)][1]['STATE_VALUE']);
            ...
            foreach($period_data as $pdata)
            {
                ...
                $period_data_RET = DBGet(DBQuery('SELECT ap.STUDENT_ID,ap.PERIOD_ID,ap.SCHOOL_DATE,ac.SHORT_NAME,ac.STATE_CODE,ap.ATTENDANCE_CODE,ap.COMMENT AS TEACHER_COMMENT FROM attendance_period ap,attendance_codes ac WHERE ap.ATTENDANCE_CODE=ac.ID AND ap.PERIOD_ID=\''.$pdata['PERIOD_ID'].'\' AND ap.STUDENT_ID=\''.$student_id.'\' AND ap.SCHOOL_DATE BETWEEN \''.$start_date.'\' AND \''.$end_date.'\''),array(),array('SCHOOL_DATE','PERIOD_ID'));  
                $day_period_data['PERIOD_ID']=$pdata['PERIOD_ID'];
                $day_period_data['SHORT_NAME']=$pdata['SHORT_NAME'];
                $days_RET[$i]['START_TIME']=$pdata['START_TIME'];
                $days_RET[$i]['END_TIME']=$pdata['END_TIME'];
                $day_period_data['STATE_CODE']=((isset($period_data_RET[$school_date][$pdata['PERIOD_ID']][1]['ATTENDANCE_CODE']))?$period_data_RET[$school_date][$pdata['PERIOD_ID']][1]['STATE_CODE']:'');
