...
if(count($auth_data)>0)
{
    if($auth_data['user_id']==$student_id && $auth_data['user_profile']=='student')
    {
        ...
        //            $user_id=UserWs('STAFF_ID');
                ...
                $username_user=DBGet(DBQuery('SELECT USERNAME FROM login_authentication WHERE USER_ID='.$user_id.' AND PROFILE_ID=\'3\''));
                ...
                if(count($to_array)>0)
                {
                    ...
                    {
                        $temp_to=array();
                                $group_check=DBGet(DBQuery('SELECT DISTINCT mgm.USER_NAME FROM mail_group mg,mail_groupmembers mgm WHERE mg.GROUP_NAME=\''.trim($ta).'\' AND mg.USER_NAME=\''.$username_user.'\' AND mg.GROUP_ID=mgm.GROUP_ID'));
        ...
        //        
        //        }    
                if($profile=='parent' || $profile=='student')
                {
                $course_periods=DBGet(DBQuery('SELECT GROUP_CONCAT(course_period_id) as COURSE_PERIOD_ID FROM schedule WHERE STUDENT_ID='.$_REQUEST['student_id']));
        ...
        //        }
                if($profile=='student')
                {
                $tmp_q=DBGet(DBQuery('SELECT DISTINCT la.USERNAME FROM login_authentication la,student_enrollment se,students_join_people sjp WHERE  se.STUDENT_ID=sjp.STUDENT_ID AND sjp.STUDENT_ID='.$user_id.' AND sjp.PERSON_ID=la.USER_ID AND la.USERNAME IS NOT NULL AND la.PROFILE_ID=4 ')); //(se.START_DATE=\'0000-00-00\' OR se.START_DATE<=\''.date('Y-m-d').'\') AND (se.END_DATE=\'0000-00-00\' OR se.END_DATE IS NULL OR se.END_DATE>=\''.date('Y-m-d').'\') AND
                foreach($tmp_q as $tmp_a)
                $tmp_arr[]=$tmp_a['USERNAME'];
                ...
                $tmp_q=DBGet(DBQuery('SELECT la.USERNAME FROM login_authentication la,staff s,staff_school_relationship ssr,user_profiles up WHERE s.PROFILE=\'admin\' AND ssr.STAFF_ID=s.STAFF_ID  AND ssr.SCHOOL_ID='.UserSchool().'  AND la.USER_ID=s.STAFF_ID AND la.PROFILE_ID=up.ID AND up.PROFILE=s.PROFILE AND la.USERNAME IS NOT NULL '));  //AND (ssr.START_DATE=\'0000-00-00\' OR ssr.START_DATE<=\''.date('Y-m-d').'\') AND (ssr.END_DATE=\'0000-00-00\' OR ssr.END_DATE IS NULL OR ssr.END_DATE>=\''.date('Y-m-d').'\')
                foreach($tmp_q as $tmp_a)
                $tmp_arr[]=$tmp_a['USERNAME'];
        ...
        //
        //        }
                $tmp_own=DBGet(DBQuery('SELECT DISTINCT la.USERNAME FROM login_authentication la WHERE la.PROFILE_ID = 3 AND la.USER_ID = '.$user_id.' AND la.USERNAME IS NOT NULL'));
                ...
                foreach($to_array as $data)
                {
                            ...
                            if(in_array(trim($data),$tmp_arr))
                            $final_arr[]=trim($data);
                    else
                            $cannot_send[]=trim($data);    
                }
