...
if(($old_school_id!=$cur_school_id) || ($old_syear!=$new_syear))
{
...
if(!$RET)
{
    ...
    {
    $teacher_info['UserMP'] = $new_marking_period_id;
    if($mp_type == 'QTR')
    {
        $RET = DBGet(DBQuery("SELECT MARKING_PERIOD_ID,TITLE FROM school_quarters WHERE SCHOOL_ID='".$cur_school_id."' AND SYEAR='".$new_syear."' ORDER BY SORT_ORDER"));
        $allMP='QTR';
...
if(($old_school_id!=$cur_school_id) || ($old_syear!=$new_syear) || ($old_marking_period_id != $new_marking_period_id))
{
    $teacher_info['UserSubject']=$RET[1]['SUBJECT_ID'];
}
else 
{
    $teacher_info['UserSubject']=$new_subject;
}
$i=0;
$data=array();
if(count($RET)>0)
...
$RET = DBGet($course);
if(($old_school_id!=$cur_school_id) || ($old_syear!=$new_syear) || ($old_marking_period_id != $new_marking_period_id) || ($old_subject != $new_subject))
{
    $teacher_info['UserCourse']=$RET[1]['COURSE_ID'];
}
else 
{
    $teacher_info['UserCourse']=$new_course_id;
}
$i=0;
$data=array();
if(count($RET)>0)
...
{
    $teacher_info['UserCoursePeriod'] = $new_course_period_id;
    $teacher_info['UserCoursePeriodVar'] = $RET[1]['ID'];
}
