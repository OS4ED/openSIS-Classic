...
$cp_data_sql = 'SELECT cp.course_period_id,cp.title,cp.total_seats,cp.total_seats,cp.filled_seats,(cp.total_seats - cp.filled_seats) as available_seats,(SELECT CONCAT(first_name,\' \',last_name) AS staff_name FROM staff WHERE staff_id = cp.teacher_id) AS teacher_name,cp.teacher_id FROM  course_periods cp';
...
echo json_encode(array('success'=>$success,'result'=>$result));
