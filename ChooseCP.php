...
if( $_REQUEST['table_name']!='' && $_REQUEST['table_name']=='course_periods')
		{
                ...
                $coursePeriods_RET = DBGet($QI);
                 $html='cp_modal_cp||';
                  ...
                  $html.='<table id="selected_course1"><tr><td></td></tr></table>';
                     $html.='<table class="table table-bordered"><tr class="bg-grey-200"><th>&nbsp;</th><th>Course Periods</th><th>Available Seats</th></tr>'; 
        ...
        if(count($coursePeriods_RET))
                {
                       ...
                       $html.='<BR>'.SubmitButtonModal('Done','done','class="btn btn-primary" ').'&nbsp;&nbsp;'.SubmitButtonModal('Clear & Exit','exit','class="btn btn-primary" ');
                }
                 $html.='</FORM>';
                }
