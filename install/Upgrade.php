...
$q1 = $dbconn->query("SELECT name,value FROM app where name='version'") or die($dbconn->error . 'error at 1');

$q2r = $dbconn->query("SELECT name,value FROM app where name='version'") or die($dbconn->error . 'error at 2');
$q2 = $q2r->fetch_assoc();
$v = $q2['value'];
if ($v == '5.0') {
    ...
    $dbconn->query('DELETE FROM custom_fields WHERE system_field=\'Y\' ') or die($dbconn->error . 'error at 3');
    $dbconn->query('TRUNCATE app') or die($dbconn->error . 'error at 4');
    ...
    $dbconn->query($app_insert) or die($dbconn->error . 'error at 96');
...
if (count($extra_tab) > 0) {
    ...
    foreach ($extra_tab as $vk => $vv) {
        ...
        $dbconn->query("drop table " . $vv) or die($dbconn->error . ' at line 209');
    }
}
...
FROM app
WHERE value='4.6' OR value='4.7' OR value LIKE '4.8%' OR value='4.9' OR value='5.0' OR value='5.1' OR value='5.2' OR value='5.3'") or die($dbconn->error . ' at line 220');
    ...
    $get_routines = $dbconn->query('SELECT routine_name,routine_type FROM information_schema.routines WHERE routine_schema=\'' . $mysql_database . '\' ') or die($dbconn->error . ' at line 229');
    while ($get_routines_arr = $get_routines->fetch_assoc()) {

        $dbconn->query('DROP ' . $get_routines_arr['routine_type'] . ' IF EXISTS ' . $get_routines_arr['routine_name']) or die($dbconn->error . ' at line 232');
    }

    $get_trigger = $dbconn->query('SELECT trigger_name FROM information_schema.triggers WHERE trigger_schema=\'' . $mysql_database . '\' ') or die($dbconn->error . ' at line 235');
    while ($get_trigger_arr = $get_trigger->fetch_assoc()) {

        $dbconn->query('DROP TRIGGER IF EXISTS ' . $get_trigger_arr['trigger_name']) or die($dbconn->error . ' at line 238');
    }

    $dbconn->query('UPDATE ' . table_to_upper('students', $version) . ' SET failed_login=0 WHERE failed_login is null') or die($dbconn->error . ' at line 241');
    if ($version != '5.2' && $version != '5.3') {
        $dbconn->query('Create table staff_new as SELECT * FROM ' . table_to_upper('staff', $version) . '') or die($dbconn->error . ' at line 243');
        $dbconn->query('TRUNCATE TABLE staff_new') or die($dbconn->error . ' at line 244');
        $dbconn->query('ALTER TABLE `staff_new` DROP `syear`, DROP `schools`, DROP `rollover_id`') or die($dbconn->error . ' at line 245');

        $dbconn->query('DROP TABLE ' . table_to_upper('staff_school_relationship', $version) . '') or die($dbconn->error . ' at line 247');
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8') or die($dbconn->error . ' at line 255');

        $sql = $dbconn->query('SELECT * FROM ' . table_to_upper('staff', $version) . ' order by staff_id asc') or die($dbconn->error . ' at line 257');
        while ($row = $sql->fetch_assoc()) {
            if ($row['username'] != '')
                $staff_sql = $dbconn->query("SELECT staff_id FROM staff_new WHERE username='" . $row['username'] . "' AND username IS NOT NULL") or die($dbconn->error . ' at line 260');
            else
                $staff_sql = $dbconn->query("SELECT staff_id FROM staff_new WHERE first_name='" . $row['first_name'] . "' AND last_name='" . $row['last_name'] . "' AND profile='" . $row['profile'] . "'") or die($dbconn->error . ' at line 262');
            ...
            ,'" . $row['title'] . "','" . $row['first_name'] . "','" . $row['last_name'] . "','" . $row['middle_name'] . "','" . $row['username'] . "','" . $row['password'] . "'
                ,'" . $row['phone'] . "','" . $row['email'] . "','" . $row['profile'] . "','" . $row['homeroom'] . "','" . $row['last_login'] . "','" . $row['failed_login'] . "','" . $row['profile_id'] . "','" . $row['is_disable'] . "')") or die($dbconn->error . ' at line 267');
                if ($row['username'] != '')
                    $st_info_sql = $dbconn->query("SELECT syear,staff_id,schools FROM " . table_to_upper('staff', $version) . " WHERE username='" . $row['username'] . "' AND username IS NOT NULL") or die($dbconn->error . ' at line 269');
                else
                    $st_info_sql = $dbconn->query("SELECT syear,staff_id,schools FROM " . table_to_upper('staff', $version) . " WHERE first_name='" . $row['first_name'] . "' AND last_name='" . $row['last_name'] . "' AND profile='" . $row['profile'] . "' AND username IS NULL") or die($dbconn->error . ' at line 271');
                ...
                while ($row1 = $st_info_sql->fetch_assoc()) {
                    ...
                    foreach ($all_school as $key => $value) {
                        ...
                        $dbconn->query('insert into ' . table_to_upper('staff_school_relationship', $version) . ' values(\'' . $staff_id . '\',\'' . $value . '\',\'' . $row1['syear'] . '\',\'0000-00-00\',\'0000-00-00\')') or die($dbconn->error . ' at line 279');
                    }



                    $dbconn->query("update attendance_completed set staff_id='" . $row['staff_id'] . "' WHERE staff_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 284');
                    $dbconn->query("update  course_periods set teacher_id='" . $row['staff_id'] . "' WHERE teacher_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 285');
                    $dbconn->query("update  course_periods set secondary_teacher_id='" . $row['staff_id'] . "' WHERE secondary_teacher_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 286');
                    $dbconn->query("update  login_records set staff_id='" . $row['staff_id'] . "' WHERE staff_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 287');
                    $dbconn->query("update missing_attendance set teacher_id='" . $row['staff_id'] . "' WHERE teacher_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 288');
                    $dbconn->query("update portal_notes set published_user='" . $row['staff_id'] . "'WHERE published_user='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 289');
                    $dbconn->query("update program_user_config set user_id='" . $row['staff_id'] . "'WHERE user_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 290');
                    $dbconn->query("update schedule_requests set with_teacher_id='" . $row['staff_id'] . "'WHERE with_teacher_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 291');

                    $dbconn->query("update teacher_reassignment set teacher_id='" . $row['staff_id'] . "'WHERE teacher_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 293');
                    $dbconn->query("update teacher_reassignment set pre_teacher_id='" . $row['staff_id'] . "'WHERE pre_teacher_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 294');
                    $dbconn->query("update teacher_reassignment set modified_by='" . $row['staff_id'] . "'WHERE modified_by='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 295');
                    $dbconn->query("update gradebook_assignments set staff_id='" . $row['staff_id'] . "' WHERE staff_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 296');
                    $dbconn->query("update gradebook_assignment_types set staff_id='" . $row['staff_id'] . "' WHERE staff_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 297');
                    $dbconn->query("update grades_completed set staff_id='" . $row['staff_id'] . "' WHERE staff_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 298');
                    $dbconn->query("update student_mp_comments set staff_id='" . $row['staff_id'] . "' WHERE staff_id='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 299');
                    $dbconn->query("update schedule set modified_by='" . $row['staff_id'] . "' WHERE modified_by='" . $row1['staff_id'] . "'") or die($dbconn->error . ' at line 300');
                }
            }
        }

        $dbconn->query('DROP TABLE ' . table_to_upper('staff', $version) . '') or die($dbconn->error . ' at line 305');
        $dbconn->query('RENAME TABLE `staff_new` TO ' . table_to_upper('staff', $version) . '') or die($dbconn->error . ' at line 306');
    }
...
//
        ...
        $dbconn->query("drop database $mysql_database") or die($dbconn->error . ' at line 346');

        $dbconn->query("CREATE DATABASE $mysql_database CHARACTER SET utf8 COLLATE utf8_general_ci") or die($dbconn->error . ' at line 348');
        mysqli_close($dbconn);
//        mysql_select_db($mysql_database);
        ...
        executeSQL($myFile, $mysql_database);
        $dbconn = new mysqli($_SESSION['server'], $_SESSION['username'], $_SESSION['password'], $_SESSION['db'], $_SESSION['port']);
        $dbconn->query("delete from app") or die($dbconn->error . ' at line 395');
...
('last_updated', 'December 01, 2013')";
        $dbconn->query($appTable) or die($dbconn->error . ' at line 402');
        $custom_insert = $dbconn->query("select count(*) from custom_fields where title in('Ethnicity','Common Name','Physician','Physician Phone','Preferred Hospital','Gender','Email','Phone','Language')") or die($dbconn->error . ' at line 403');
...
('text', NULL, 'Language', 8, NULL, 1, 'Y', NULL, NULL, NULL);";
            $dbconn->query($custom_insert) or die($dbconn->error . ' at line 417');
        }
        $login_msg = $dbconn->query("SELECT COUNT(*) as tot FROM login_message WHERE 1") or die($dbconn->error . ' at line 419');
...
(1, 'This is a restricted network. Use of this network, its equipment, and resources is monitored at all times and requires explicit permission from the network administrator. If you do not have this permission in writing, you are violating the regulations of this network and can and will be prosecuted to the fullest extent of law. By continuing into this system, you are acknowledging that you are aware of and agree to these terms.', 'Y')";
            $dbconn->query($login_msg) or die($dbconn->error . ' at line 425');
        }

        $syearqr = $dbconn->query("select MAX(syear) as year, MIN(start_date) as start from school_years") or die($dbconn->error . ' at line 428');
...
//=============================4.8.1 To 4.9===================================
        ...
        (" . $max_syear . ",'Rolled over','ROLL','Roll'); ";
            $dbconn->query($up_sql) or die(show_error1() . ' Show Error 3');
            ...
            $dbconn->query($up_sql) or die(show_error1() . ' Show Error 4');

            $up_sql = "INSERT INTO program_config (program, title, value) VALUES
            ('MissingAttendance', 'LAST_UPDATE','" . $start_date . "'); ";
            $dbconn->query($up_sql) or die(show_error1() . ' Show Error 5');
            ...
            $dbconn->query($up_sql) or die(show_error1() . ' Show Error 6');
        }
//====================================================================
                            ...
                            <div class="panel-body">
                                <div class="installation-steps-wrapper">
                                    <div class="installation-steps text-center">
                                        <h2 class="text-center text-danger no-margin" style="padding: 30px;">The database you have chosen is not compliant with openSIS-CE version 4.7 or 4.8X or 4.9 or 5.0 or 5.1 or 5.2 or 5.3 or 6.0 We are unable to proceed.</h2>
