...
//                echo "<br>";
     ...
     $file_display=$sub;
                 ...
                 echo '<tr class="'.$gridClass.'">
                          <td><a href="DownloadWindow.php?down_id='.$file_val['ID'].'">'.str_replace("opensis_space_here"," ",$file_display).'</a></td>
                          ';

                  if(AllowEdit ())
                     {
                      echo '<td><input type="hidden" name="del" value="'.$file_val['ID'].'"/>
                          <a href=Modules.php?modname='.$_REQUEST[modname].'&title='.base64_encode($file_val['NAME']).'&include='.$_REQUEST['include'].'&modfunc=delete&del='.$file_val['ID'].'><img src="assets/remove_button.gif" border=0 vspace=0></a>
                              </td>';
                     }
