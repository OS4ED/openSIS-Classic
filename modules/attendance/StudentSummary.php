...
if (isset($_REQUEST['student_id'])) {
    ...
    if ($count_student_RET[1]['NUM'] > 1) {
        DrawHeaderHome('<div class="panel"><div class="panel-heading"><h6 class="panel-title">Selected Student : ' . $RET[1]['FIRST_NAME'] . '&nbsp;' . ($RET[1]['MIDDLE_NAME'] ? $RET[1]['MIDDLE_NAME'] . ' ' : '') . $RET[1]['LAST_NAME'] . '&nbsp;' . $RET[1]['NAME_SUFFIX'] . '<a class="heading-elements-toggle"><i class="icon-more"></i></a></h6> <div class="heading-elements"><span class="heading-text"><A HREF=Modules.php?modname=' . $_REQUEST['modname'] . '&search_modfunc=list&next_modname=' . $_REQUEST['modname'] . '&ajax=true&bottom_back=true&return_session=true&&day_start=' . $_REQUEST[day_start] . '&&month_start=' . $_REQUEST[month_start] . '&&year_start=' . $_REQUEST[year_start] . '&&period_id=' . $_REQUEST[period_id] . '&&myclasses=' . $_REQUEST[myclasses] . '&&chk=1 target=body><i class="icon-square-left"></i> Back to Student List</A></span><div class="btn-group heading-btn"><A HREF=Side.php?student_id=new&modcat=' . $_REQUEST['modcat'] . ' class="btn btn-danger btn-xs">Deselect</A></div></div></div></div>');
...
if ($_REQUEST['search_modfunc'] || $_REQUEST['student_id'] || UserStudentID() || User('PROFILE') == 'parent' || User('PROFILE') == 'student') {
    ...
    DrawHeaderHome('<div class="form-inline clearfix"><div class="col-md-12">' . PrepareDateSchedule($start_date, 'start') .'<div class="form-group">&nbsp; - &nbsp;</div>' . PrepareDateSchedule($end_date, 'end') . '<div class="form-group"> &nbsp;<INPUT type=submit name=absence_go class="btn btn-primary" value=Go></div><div class="form-group">', $period_select . '</div><div class="form-group">' . $myclasses . '</div></div></div>');
    echo '</div>';
    echo '</div>';
    echo '</FORM>';
}
